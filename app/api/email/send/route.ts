...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create Gmail client and send message
    const client = new GmailClient(accessToken, tokens.refreshToken)

    // Get user email for "From" field - accountId is typically the email address
    // If not, get it from integrations document
    ...
    if (threadId) {
      try {
        ...
        // Continue without reply headers
      }
    }

    const messageId = await client.sendMessage(fromEmail, to, subject, body, {
      cc,
      bcc,
      threadId,
      inReplyTo,
